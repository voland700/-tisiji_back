...
document.getElementById('askBtn').addEventListener('click', function(event){
	event.preventDefault();
	let modal = tmplModal.content.cloneNode(true).querySelector('.modal_wrap');
    const fancyModal = new Fancybox([{ src: modal, type: "html" }]);
	const inpName = modal.querySelector('#inpName');
	const inpMail = modal.querySelector('#inpMail');
	const message = modal.querySelector('#message');
	const modalBtn = modal.querySelector('#modalBtn');
	const askNameValid = modal.querySelector('#askNameValid');
	const askMailValid = modal.querySelector('#askMailValid');
	let valid = true;
            ...
            getCоntеnt('/make-a-question', {
                ...
                if(data == 200){
                    ...
                    Swal.fire({
                        ...
                        text: 'Мы обязательно Вам ответим!',
                        icon: 'success',
                        timer: 5000,
                    });
                }else{
